	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata_base v)
			{
			    ...
			    float invScaleDepth = 1.0f / _ScaleDepth;
				...
				float3 start = _CameraPos.xyz;
				float tmpHeight = length(start);
				float tmpDepth = exp(_ScaleOverScaleDepth * (_InnerRadius - _CameraHeight));
				float startAngle = dot(ray, start) / tmpHeight;
				float startOffset = tmpDepth * scale(startAngle, _ScaleDepth);
				...
				o.dirToCam = _CameraPos.xyz - vertPos;
